    ...
    sl.get<DBHelper>().getAccountSpecificRecords(account).then((data) {
      setState(() {
        ...
        for (var tx in _txRecords) {
          ...
          // }
          print("memo: ${tx.memo} send_block ${tx.block} recv_block ${tx.link} ${tx.is_request}");
          ...
          if (!tx.is_request && isEmptyOrNull(tx.link) && !isEmptyOrNull(tx.block)) {
...
// Transaction Card/List Item
  Widget _buildUnifiedCard(dynamic item, Animation<double> animation, String displayName, BuildContext context, {TXData txDetails}) {
    ...
    if (txDetails != null) {
      if (txDetails.record_type == "gift" ||
          txDetails.record_type == RecordTypes.GIFT_ACK ||
      ...
      onTriggered: (preempt) {
        if (preempt) {
          ...
          sl.get<DBHelper>().getUserOrContactWithAddress(item.account).then((user) {
            ...
            String address = isPaymentRequest ? item.from_address : item.account;
            if (user is User) {
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    (isPaymentRequest && item.memo != null)
                        ? Container(
  ...
  Widget _buildUnifiedItem(BuildContext context, int index, Animation<double> animation) {
    ...
    if (displayName == null) {
      log.d(_unifiedListMap[StateContainer.of(context).wallet.address][localIndex].account);
      log.d("displayName is null?");
      displayName = "null?";
    }
...
class _PaymentDetailsSheetState extends State<PaymentDetailsSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    if (walletAddress == txDetails.to_address && txDetails.is_request && !txDetails.is_acknowledged) {
      is_unacknowledged_request = true;
    }

    if (!txDetails.is_acknowledged && !txDetails.is_request && txDetails.memo.isNotEmpty) {
