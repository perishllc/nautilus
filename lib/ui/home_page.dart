...
class _AppHomePageState extends State<AppHomePage> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  Future<void> _addSampleContact() async {
    ...
    if (!contactAdded) {
      ...
      final bool nameExists = await sl.get<DBHelper>().contactExistsWithName(nautilusDonationsNickname);
      if (nameExists) {
        return;
      }
        ...
        for (TXData tx in _txRecords) {
          ...
          if (tx.is_memo && isEmpty(tx.link) && isNotEmpty(tx.block)) {
            if (_historyListMap[StateContainer.of(context).wallet!.address] != null) {
              ...
              for (AccountHistoryResponseItem histItem
  ...
  Future<void> generateUnifiedList({bool fastUpdate = false}) async {
    ...
    unifiedList = List<dynamic>.from(
        historyList.where((AccountHistoryResponseItem element) => ![BlockTypes.CHANGE, BlockTypes.OPEN].contains(element.subtype)).toList());
    ...
    if (_searchController.text.isNotEmpty) {
      _unifiedListMap[StateContainer.of(context).wallet!.address]!
          .items
          .where((item) =>
              _unifiedListMap[StateContainer.of(context).wallet!.address]!.items.indexOf(item) !=
          ...
          children: <Widget>[
            Column(
              children: <Widget>[
                ...
                Row(
                  children: <Widget>[
                    AppButton.buildAppButton(context, AppButtonType.PRIMARY, AppLocalization.of(context)!.viewDetails,
                  ...
                  Row(
                    children: <Widget>[
                      AppButton.buildAppButton(context, AppButtonType.PRIMARY_OUTLINE,
                          AppLocalization.of(context)!.payRequest, Dimens.BUTTON_TOP_EXCEPTION_DIMENS, onPressed: () {
                        Navigator.of(context).popUntil(RouteUtils.withNameLike("/home"));
